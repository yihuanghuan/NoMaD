...
# 4. 可视化函数 (支持多条轨迹)
# ==========================================
def visualize_result(img_path, waypoints_list, output_name="result_fixed.jpg"):
    """
    绘制多条动作序列轨迹
    Args:
        img_path: 图片路径
        waypoints_list: list of waypoints, 每个元素是 (8, 2) 的 numpy array
        output_name: 输出文件名
    """
    ...
    # 绘制起点 (所有轨迹共享同一个起点)
    cv2.circle(img, (start_x, start_y), 8, (0, 0, 255), -1) 
    ...
    # 绘制所有轨迹
    for traj_idx, waypoints in enumerate(waypoints_list):
        ...
        for i in range(len(waypoints)):
            ...
            # 绘制轨迹点和线
            cv2.circle(img, curr_pt, 3, color, -1) 
            cv2.line(img, prev_pt, curr_pt, color, 2)
            prev_pt = curr_pt
        
    cv2.imwrite(output_name, img)
    print(f"\n[完成] 可视化结果已保存为 {output_name} (包含 {len(waypoints_list)} 条轨迹)")
...
def test_vision():
    ...
    for img_name in test_images:
        ...
        try:
            ...
            run_inference(model, obs_batch, goal_batch, mask_explore, device, img_path, out_path)
            
        except Exception as e:
            print(f"处理 {img_name} 时出错: {e}")

def run_inference(model, obs_batch, goal_batch, mask_batch, device, img_path, out_name, num_samples=5):
    """
    执行推理并生成多条轨迹
    Args:
        num_samples: 生成的轨迹数量,默认5条
    """
    ...
    with torch.no_grad():
        ...
        if dist_val < STOP_THRESHOLD:
            ...
            waypoints_list = [np.zeros((8, 2)) for _ in range(num_samples)]
            visualize_result(img_path, waypoints_list, out_name)
            return
